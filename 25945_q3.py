...
path_gd = [P(angles_k)]        # optional: record the path of end-effector positions
dist = calculate_euclidean_distance(angles_k)

# Main GD loop: update angles along the negative gradient until the target is reached closely.
...
# Output final joint configuration in degrees only (as required).
print(f"Final Angles (deg): {[degrees(a) for a in angles_k]}")
...
path_gdm = [P(angles_k)]                                   # optional: record path
dist = calculate_euclidean_distance(angles_k)              # compute initial distance

# Main GDM loop: apply velocity-augmented updates until convergence.
while (dist > max_distance):
